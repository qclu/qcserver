...
vmap  :<BS><BS><BS><BS><BS>SetcommentV
nmap  :Setcomment
nmap  :GoDef
nmap  :GoRef
snoremap <silent>  c
...
nmap <F4> :YcmDiags
nmap <F6> :source .back.vim
nmap <F5> :mksession! .back.vim
imap  :Setcomment
...
silent only
cd ~/workspace/gobench/src/qcserver/src
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +188 models/dbSync.go
badd +10 util/log/log.go
badd +156 main.go
badd +22 util/log/log_test.go
badd +60 models/administrators.go
badd +38 models/hospital.go
badd +6 models/consts.go
badd +1 util/time/time.go
argglobal
silent! argdel *
argadd models/dbSync.go
edit models/consts.go
...
exe '3resize ' . ((&lines * 43 + 25) / 51)
exe 'vert 3resize ' . ((&columns * 164 + 118) / 237)
exe '4resize ' . ((&lines * 1 + 25) / 51)
exe 'vert 4resize ' . ((&columns * 164 + 118) / 237)
exe 'vert 5resize ' . ((&columns * 40 + 118) / 237)
argglobal
enew
file NERD_tree_5
let s:cpo_save=&cpo
...
setlocal spellfile=
setlocal spelllang=en
setlocal statusline=%#StatusLineNC#[Name]\ consts.go
setlocal suffixesadd=
setlocal noswapfile
setlocal synmaxcol=3000
if &syntax != 'tagbar'
